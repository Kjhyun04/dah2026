...
logs GET은 신뢰 경계를 넓히지 않는다. 따라서 ``docker logs``가 메커니즘으로 유지된다(프록시
...
# 접두 대신 config ``ran_cidr``에 대해 CIDR 검사되므로, 파서는 다른 RAN CIDR을 가진
...
def parse_mongo_line(line: str, ran_cidr: str = _DEFAULT_RAN_CIDR) -> Optional[dict]:
    ...
    Null-safe: None/"" -> None. 이식성: 이상-소스 판정은 (config 기반) ``ran_cidr``에
