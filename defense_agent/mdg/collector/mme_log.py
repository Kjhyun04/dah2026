...
먼저 ANSI-strip, 그다음 regex; 타임스탬프 형식 MM/DD HH:MM:SS.mmm은 parse_epc_ts로
고정된다.

이는 tests/verify_parsers.py가 실행하는 파서 집합의 P4-5 멤버다. 순수 관측,
...
class MmeLogTail(BaseCollector):
    """그래프 외부 데몬: docker logs epc_mme를 tail하고 attach/IMSI 이벤트를
