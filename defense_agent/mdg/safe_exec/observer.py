...
메커니즘별 확인 (규칙은 RECOVERY_PRIORS 를 통해 response_tool 로 해석):
  ...
  - nsenter_input_drop (pfcp_firewall / mongo_acl): enforce_at 컨테이너의 netns 안에
    회복 강제가 설치되어 있다. 신호 = INPUT 체인 DROP 규칙 설치됨
    ...
    규칙 존재는 DROP 이 실제 설치되는 즉시 확인되므로 재작동이 멱등적으로 억제된다.
  - send_signed_mode (signed_*): 다운링크 텔레메트리 rel_alt 이 목표
...
Fail-safe: 미해석 대상 / 의존성 결여 / DRY / 에러는 모두 False (미확인)를 낳으며,
