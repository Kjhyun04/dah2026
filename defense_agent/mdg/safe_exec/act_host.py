...
메커니즘 분리(PS-1 권한분리):
  - nsenter_input_drop (AUTO)  : nsenter --target <pid> --net -- iptables ...로
    대상의 network namespace 안에 적용되는 netns INPUT DROP(C-1). --net만으로는
...
리스트)를 만들어 유일한 spawn 지점인 Backend.run에 넘긴다. 최소 권한: netns DROP은
...
def drop_argv(pid: Optional[int], src_ip: str, *, revert: bool = False,
    ...
    누수-0 정합). -I는 DROP을 삽입한다; revert=True는 대응하는 -D 삭제를 방출한다.
    """
...
class ActHost:
    ...
    def apply_input_drop(self, pid: Optional[int], src_ip: str,
        ...
        """Backend.run으로 대상 netns 안에 src_ip에 대한 DROP을 삽입한다(allow_live가
