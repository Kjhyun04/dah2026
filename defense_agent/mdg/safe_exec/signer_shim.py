...
def emit_signed(intent, backend=None) -> SignerEmit:
    """Emit a signed-mode command WITHOUT holding a key.

    allow_live=False (default / operator-go 유보): compute ONLY the command_digest (the token
    material) and return a DRY result. A live promotion must DELEGATE to ``gcs_c2`` (which
    already holds its own key) over an authenticated channel — this function must never open,
    read, or reference any signing-key path. The ``backend`` arg is accepted for symmetry but the key-free
    emitter performs no local spawn in MVP (delegation is out-of-band, operator-go).
    """
    ...
    # Live promotion: authorization must have been operator-approved (OperatorGate.verify) and the
    # signature is issued by the gcs_c2 delegate out-of-band. MDG never re-mounts the key.
    return SignerEmit(ok=True, dry_run=False, command_digest=dg,
                      note="delegate to gcs_c2 signing endpoint (authenticated channel, operator-go)")
